        ...
        <div Id="main">
      	    ...
      	    <article class="post">
                  ...
                  if (isset($_SESSION['IdEtu']) && isset($_SESSION['nomEtu'])) {
                        ...
                        echo "Veuillez vous connecter avant de visualiser les informations sur l'type";
                    }
                    ?>

                    <!-- Afficher les livres de cet type -->
                    ...
                    if(isset($_GET['IdType'])) {
                        ...
                        $req = "SELECT IdAuteur, IdType, IdLivre, titreLivre, anneePubli FROM Livre WHERE IdType = $IdType ORDER BY $orderBy;";
                        $res = mysqli_query($conn, $req);
                    
                        // 开始表格
                        echo '<table>';
                        echo '<tr><th>Titre Livre<a href="type2.php?IdType=' . $IdType . '&sort=titre" style="color: gray;"></br>Classer par titre</a></th>
			...
			<div class="sidemenu">
				...
				<ul>
                    ...
                    if (isset($_SESSION['IdEtu'])&&isset($_SESSION['nomEtu'])){
						...
						// 查询用户已借阅的书籍
						$req = "SELECT Livre.titreLivre, Livre.anneePubli, Livre.IdLivre, Livre.IdType
								FROM Livre
						...
						if (mysqli_num_rows($res) > 0) {
							while ($row = mysqli_fetch_array($res)) {
                                ...
                                echo '</br><a href="retourner1.php?IdLivre=' . $row['IdLivre'] . '&titreLivre=' . $row['titreLivre'] . '&IdType=' . $row['IdType'] . '">Retour</a></li>';
							}
						} else {
